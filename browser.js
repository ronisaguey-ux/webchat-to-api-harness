        ...
        try {
            ...
            if (stillFull) {
                ...
                if (btnInfo) {
                    ...
                    // 09-15: the composer TEXT is not a reliable "unsent" signal -
                    // gemini keeps its contenteditable populated after a successful
                    // send until the next render, so a text-only check turned WORKING
                    // sends into hard failures (measured: 17/23 received before this
                    // check existed, 3/35 after). Confirm against the THREAD instead:
                    // if the conversation grew, the message went and we carry on.
                    if (stillUnsent) {
                        const grew = await page.evaluate(() => {
                            try {
                                const before = window.__wsSendProbeCount || 0;
                                const rows = document.querySelectorAll(
                                    '[data-message-author-role], .chat-content-item-assistant, message-content, .query-text, .conversation-turn');
                                const now = rows.length;
                                window.__wsSendProbeCount = now;
                                return now > before;
                            } catch { return false; }
                        }).catch(() => false);
                        if (grew) {
                            console.log('ℹ️ composer still holds text but the thread grew — the send DID go through');
                            return;
                        }
